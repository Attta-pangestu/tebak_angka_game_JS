 //inisialiasi variabel untuk menampung elemen dokumen
    ...
    function getAnswer() {
      ...
      for (let i = 0; i < answer.length; i++) {
        let j = Math.floor(Math.random() * (i + 2));
        let tmp = answer[i];
        answer[i] = answer[j];
        answer[j] = tmp;
    ...
    playButton.addEventListener('click', getAnswer);
    ...
    function checkAnswer(ans) {
      const trueAnswer = sessionStorage.getItem(sessionAnswerKey);
      if (ans === trueAnswer) {
        duringGameDisplay.setAttribute('hidden', true);
        afterGameDisplay.removeAttribute('hidden');
        sessionTrueAnswerField.innerText = trueAnswer;
        updateScore();
      }
      else {
        const previousAttemptCount = parseInt(sessionStorage.getItem(sessionUserAttemptsKey));
        sessionStorage.setItem(sessionUserAttemptsKey, previousAttemptCount + 1);
        sessionUserAttemptsField.innerText = sessionStorage.getItem(sessionUserAttemptsKey);
